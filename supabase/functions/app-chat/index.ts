...
serve(async (req) => {
  ...
  try {
    ...
    /* ── Mode: wizard-setup (streaming via OpenRouter free models) ── */
    if (mode === "wizard-setup") {
      ...
      let wizardResp: Response | null = null;
      for (const model of FREE_MODELS) {
        try {
          ...
          if ([400, 404, 429, 500, 502, 503].includes(r.status)) continue;
          wizardResp = r;
          break;
        } catch { continue; }
      }
      ...
      if (!wizardResp?.ok) {
        // Fallback: stream a static error message in SSE format
        const encoder = new TextEncoder();
        const stream = new ReadableStream({
          start(ctrl) {
            const msg = "Desculpe, o serviço de IA está temporariamente indisponível. Tente novamente em instantes.";
            msg.split(" ").forEach((w, i, arr) => {
      ...
      return new Response(wizardResp.body, {
        headers: { ...corsHeaders, "Content-Type": "text/event-stream" },
      });
    }

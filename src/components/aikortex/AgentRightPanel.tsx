            ...
            <ScrollArea className="flex-1">
              <div className="p-6 max-w-lg space-y-8">
                ...
                {settingsNav === "machine" && (
                  <div className="space-y-6">
                    <div>
                      <h2 className="text-lg font-bold text-foreground">Modelo</h2>
                      <p className="text-sm text-muted-foreground mt-1">Selecione o modelo de IA usado pelo agente.</p>
                    </div>

                    {(() => {
                      const llmProviders = ["OpenAI", "Anthropic", "Gemini"] as const;
                      const hasLLMKey = llmProviders.some(p => connectorKeys[p]?.configured);
                      ...
                      return (
                        <>
                          ...
                          <div className="space-y-2">
                            <h3 className="text-sm font-semibold text-foreground">Provedores</h3>
                            <p className="text-xs text-muted-foreground">Conecte um provedor para selecionar modelos.</p>
                            <div className="space-y-1">
                              {llmProviders.map(provider => {
                                const isConnected = connectorKeys[provider]?.configured;
                                const integration = INTEGRATIONS.find(i => i.label === provider);
                                return (
                                  <div key={provider} className="flex items-center justify-between py-2.5 px-3 rounded-lg hover:bg-muted/50 transition-colors">
                                    <div className="flex items-center gap-3">
                                      {integration?.logo && (
                                        <img src={integration.logo} alt={provider} className="w-6 h-6 rounded object-contain shrink-0" onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
                                      )}
                                    ...
                                    {!isConnected && integration && (
                                      <Button variant="ghost" size="sm" className="text-xs gap-1" onClick={() => handleConnectIntegration(integration)}>
                                        + Conectar
                                      </Button>
                                    )}
                                    {isConnected && integration && (
                                      <Button variant="outline" size="sm" className="text-xs gap-1" onClick={() => handleConnectIntegration(integration)}>
                                        <Settings className="w-3 h-3" /> Gerenciar
                                      </Button>
                                    )}
                                  </div>
                          ...
                          {hasLLMKey && availableModels.length > 0 && (
                            <div className="space-y-2">
                              <h3 className="text-sm font-semibold text-foreground">Modelo selecionado</h3>
                              <p className="text-xs text-muted-foreground">O modelo de IA usado pelo agente em produção e testes.</p>
                          ...
                          {!hasLLMKey && (
                            <div className="rounded-lg border border-border bg-muted/30 p-4 text-center space-y-2">
                              ...
                              <p className="text-sm text-muted-foreground">Conecte um provedor acima para selecionar o modelo.</p>
                            </div>
                          )}
                        </>
                      );

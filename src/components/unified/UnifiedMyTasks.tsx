import { TaskEngineItem, STATUS_CONFIG, PRIORITY_CONFIG, TEAM_MEMBERS } from "@/types/task-engine";
...
const UnifiedMyTasks = ({ items, onItemClick }: Props) => {
  const [currentUser, setCurrentUser] = useState("Ana Silva");
  const tasks = items.filter((i) => i.task_type === "task");
  const myTasks = tasks.filter((t) => t.assignee === currentUser && t.status !== "completed");
  ...
  const completed = tasks.filter((t) => t.assignee === currentUser && t.status === "completed");
        ...
        <div key={title} className="glass-card rounded-xl overflow-hidden">
          ...
          <div className="divide-y divide-border/30">
            {sectionTasks.map((task) => {
              ...
              return (
                <div key={task.id} onClick={() => onItemClick(task)} className="flex items-center gap-3 px-4 py-3 hover:bg-accent/40 cursor-pointer transition-colors">
                  <div className="w-1.5 h-8 rounded-full" style={{
                    backgroundColor: task.priority === "urgent" ? "hsl(var(--destructive))" :
                      ...
                      task.priority === "medium" ? "hsl(var(--info))" : "hsl(var(--muted-foreground))"
                  }} />
                  ...
                  <span className="text-[10px] text-muted-foreground">{format(new Date(task.dueDate), "dd MMM", { locale: ptBR })}</span>
                </div>
              );
            })}
          </div>

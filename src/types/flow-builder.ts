...
export const FLOW_TEMPLATES: FlowTemplate[] = [
  {
    ...
    nodes: [
      nd("s1", "Mensagem recebida", "trigger", "💬", "Nova mensagem no WhatsApp", "hsl(142,71%,45%)", { channel: "whatsapp" }, pos(400, 50)),
      nd("s2", "Boas-vindas", "message", "✉️", "Envia saudação", "hsl(217,91%,60%)", { text: "Olá! 👋 Como posso ajudar?" }, pos(400, 180)),
      nd("s3", "Capturar nome", "message", "📝", "Pede o nome", "hsl(217,91%,60%)", { variable: "nome", validation: "text" }, pos(400, 310)),
      nd("s4", "Capturar email", "message", "📝", "Pede o email", "hsl(217,91%,60%)", { variable: "email", validation: "email" }, pos(400, 440)),
      nd("s5", "Agente IA", "agent", "🤖", "Qualifica o lead", "hsl(330,81%,60%)", { agentId: "sdr", temperature: 0.7 }, pos(400, 570)),
      nd("s6", "É qualificado?", "condition", "🔀", "Verifica score", "hsl(45,93%,47%)", { expression: '{{score}} >= 7' }, pos(400, 700)),
      nd("s7", "Criar lead CRM", "integration", "💼", "Salva no CRM", "hsl(199,89%,48%)", { provider: "hubspot", action: "create_lead" }, pos(250, 850)),
      nd("s8", "Transferir humano", "action", "👤", "Passa para vendedor", "hsl(262,83%,58%)", { department: "vendas" }, pos(250, 980)),
      nd("s9", "Enviar conteúdo", "message", "✉️", "Envia material educativo", "hsl(217,91%,60%)", { text: "Enquanto isso, confira nosso material..." }, pos(550, 850)),
    ],
    edges: [
  ...
  },
  {
    ...
    nodes: [
      nd("c1", "Mensagem recebida", "trigger", "💬", "Nova mensagem", "hsl(142,71%,45%)", { channel: "any" }, pos(400, 50)),
      nd("c2", "Detectar intenção", "agent", "🧠", "Classifica assunto", "hsl(330,81%,60%)", { intents: ["duvida", "reclamacao", "elogio"] }, pos(400, 180)),
      nd("c3", "Base de conhecimento", "agent", "📚", "Busca resposta", "hsl(330,81%,60%)", { knowledgeBaseId: "" }, pos(400, 310)),
      nd("c4", "Resolveu?", "condition", "🔀", "IA resolveu?", "hsl(45,93%,47%)", { expression: '{{resolved}} == true' }, pos(400, 440)),
      nd("c5", "Enviar resposta", "message", "✉️", "Resposta automática", "hsl(217,91%,60%)", { text: "{{ai_response}}" }, pos(250, 580)),
      nd("c6", "Transferir humano", "action", "👤", "Atendente humano", "hsl(262,83%,58%)", { department: "suporte" }, pos(550, 580)),
      nd("c7", "Pesquisa satisfação", "message", "📝", "Avalia atendimento", "hsl(217,91%,60%)", { variable: "satisfacao", validation: "number" }, pos(400, 720)),
    ],
    edges: [
  ...
  },
  {
    ...
    nodes: [
      nd("o1", "Novo cliente", "trigger", "⚡", "Evento de compra", "hsl(142,71%,45%)", { event: "purchase_completed" }, pos(400, 50)),
      nd("o2", "Email boas-vindas", "action", "📧", "Envia email de boas-vindas", "hsl(262,83%,58%)", { to: "{{email}}", subject: "Bem-vindo!", body: "" }, pos(400, 180)),
      nd("o3", "Aguardar 1 dia", "delay", "⏳", "Espera 24h", "hsl(25,95%,53%)", { duration: 1, unit: "days" }, pos(400, 310)),
      nd("o4", "WhatsApp tutorial", "integration", "📱", "Envia tutorial via WhatsApp", "hsl(199,89%,48%)", { template: "onboarding_tutorial" }, pos(400, 440)),
      nd("o5", "Aguardar 3 dias", "delay", "⏳", "Espera 3 dias", "hsl(25,95%,53%)", { duration: 3, unit: "days" }, pos(400, 570)),
      nd("o6", "Check-in IA", "agent", "🤖", "IA verifica satisfação", "hsl(330,81%,60%)", { agentId: "cs", temperature: 0.5 }, pos(400, 700)),
    ],
    edges: [
  ...
  },
  {
    ...
    nodes: [
      nd("a1", "Carrinho abandonado", "trigger", "⚡", "Evento de abandono", "hsl(142,71%,45%)", { event: "cart_abandoned" }, pos(400, 50)),
      nd("a2", "Aguardar 30min", "delay", "⏳", "Espera 30 min", "hsl(25,95%,53%)", { duration: 30, unit: "minutes" }, pos(400, 180)),
      nd("a3", "Lembrete WhatsApp", "message", "✉️", "Lembra do carrinho", "hsl(217,91%,60%)", { text: "Ei {{nome}}, você esqueceu itens no carrinho! 🛒" }, pos(400, 310)),
      nd("a4", "Aguardar 1 dia", "delay", "⏳", "Espera 1 dia", "hsl(25,95%,53%)", { duration: 1, unit: "days" }, pos(400, 440)),
      nd("a5", "Comprou?", "condition", "🔀", "Verificar status", "hsl(45,93%,47%)", { expression: '{{purchased}} == true' }, pos(400, 570)),
      nd("a6", "Encerrar", "action", "🛑", "Fim do fluxo", "hsl(262,83%,58%)", {}, pos(250, 710)),
      nd("a7", "Oferecer desconto", "message", "✉️", "Cupom de desconto", "hsl(217,91%,60%)", { text: "Use o cupom VOLTE10 e ganhe 10% off! 🎉" }, pos(550, 710)),
    ],
    edges: [
  ...
  },
  {
    ...
    nodes: [
      nd("m1", "Mensagem recebida", "trigger", "💬", "Pedido de reunião", "hsl(142,71%,45%)", { channel: "any", keyword: "agendar" }, pos(400, 50)),
      nd("m2", "Agente IA", "agent", "🤖", "Identifica necessidade", "hsl(330,81%,60%)", { agentId: "sdr", temperature: 0.5 }, pos(400, 180)),
      nd("m3", "Capturar data", "message", "📝", "Pede data preferida", "hsl(217,91%,60%)", { variable: "data_reuniao", validation: "text" }, pos(400, 310)),
      nd("m4", "Criar evento", "integration", "📅", "Agenda no Google Calendar", "hsl(199,89%,48%)", { provider: "google_calendar", action: "create_event" }, pos(400, 440)),
      nd("m5", "Confirmação", "message", "✉️", "Confirma o agendamento", "hsl(217,91%,60%)", { text: "Reunião agendada para {{data_reuniao}} ✅" }, pos(400, 570)),
    ],
    edges: [
  ...
  },
  {
    ...
    nodes: [
      nd("n1", "Agendamento", "trigger", "⏰", "Dispara periodicamente", "hsl(142,71%,45%)", { cron: "0 10 * * 1" }, pos(400, 50)),
      nd("n2", "Enviar NPS", "message", "📝", "Pergunta de 0-10", "hsl(217,91%,60%)", { variable: "nps_score", validation: "number" }, pos(400, 180)),
      nd("n3", "Análise sentimento", "agent", "😊", "Analisa a nota", "hsl(330,81%,60%)", {}, pos(400, 310)),
      nd("n4", "Score >= 9?", "condition", "🔀", "Promotor?", "hsl(45,93%,47%)", { expression: '{{nps_score}} >= 9' }, pos(400, 440)),
      nd("n5", "Agradecer", "message", "✉️", "Agradece o feedback", "hsl(217,91%,60%)", { text: "Obrigado pelo feedback! 🎉" }, pos(250, 580)),
      nd("n6", "Salvar planilha", "integration", "📊", "Registra no Sheets", "hsl(199,89%,48%)", { spreadsheetId: "", range: "NPS!A:C" }, pos(250, 720)),
      nd("n7", "Alerta equipe", "action", "👤", "Notifica CS", "hsl(262,83%,58%)", { department: "cs" }, pos(550, 580)),
    ],
    edges: [

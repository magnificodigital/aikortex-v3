        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">Personalize a identidade visual da sua agência</p>
            </div>
          </div>
                    ...
                    <div key={section.id} className="border-b border-border last:border-b-0">
                      {section.type === "hero" ? (
                        <div className="p-8 text-center" style={{ background: `linear-gradient(135deg, ${colors.primary}, ${colors.accent})` }}>
                          ...
                          <button className="mt-4 px-4 py-2 rounded-lg text-xs font-semibold bg-white/20 text-white backdrop-blur border border-white/20">
                            Começar agora
                          </button>
                        </div>
                    ...
                    <div key={link.id} className={`glass-card rounded-lg p-3 space-y-2 transition-opacity ${link.active ? "" : "opacity-50"}`}>
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-center gap-0.5">
                          <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => moveBioLink(link.id, "up")} disabled={idx === 0}>
                            <ArrowUp className="w-3 h-3" />
                          </Button>
                          <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => moveBioLink(link.id, "down")} disabled={idx === bioLinks.length - 1}>
                            <ArrowDown className="w-3 h-3" />
                          </Button>
                          <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => updateBioLink(link.id, "active", !link.active)}>
                            <Eye className={`w-3 h-3 ${link.active ? "" : "text-muted-foreground"}`} />
                          </Button>
                          <Button variant="ghost" size="icon" className="h-6 w-6 text-destructive" onClick={() => removeBioLink(link.id)}>
                            <Trash2 className="w-3 h-3" />
                          </Button>
                        </div>
                      </div>
                    ...
                    <div className="p-6 text-center space-y-3">
                      <div className="w-16 h-16 rounded-full mx-auto border-2 border-white/30 overflow-hidden bg-white/10 flex items-center justify-center">
                        {logoUrl ? (
                          ...
                          <span className="text-2xl font-bold text-white">{bioTitle.charAt(0)}</span>
                        )}
                      </div>
                      <div>
                    ...
                    <div className="px-4 pb-6 space-y-2">
                      {bioLinks.filter(l => l.active).map(link => (
                        <a
                          ...
                          className="flex items-center gap-3 px-4 py-3 rounded-xl bg-white/10 backdrop-blur border border-white/10 text-white hover:bg-white/20 transition-colors"
                        >
